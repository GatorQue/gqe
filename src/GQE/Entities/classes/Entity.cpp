...
namespace GQE
{
	std::vector<Entity*> Entity::mEntityList;
	...
	*/
	...
	void Entity::AddEntity(typeEntityID theEntityID)
	{
		mEntityList.push_back(new Entity(theEntityID));
	}
	/*
	void Entity::AllHandleEvents(sf::Event theEvent)
	{
	...
	*/
	...
	void Entity::AllUpdateVariable(float theElapsdTime)
	{
		std::vector<Entity*>::iterator it;
		for(it=mEntityList.begin();it!=mEntityList.end();++it)
		{
